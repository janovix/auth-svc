...
describe("Internal Organizations Routes", () => {
	...
	describe("POST /internal/organizations/:id/invitations", () => {
		it("validates email is required", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/test-org-id/invitations",
				{
					method: "POST",
		...
		it("returns 404 for non-existent organization", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/non-existent-org/invitations",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	describe("DELETE /internal/organizations/:id/members/:memberId", () => {
		it("returns 404 for non-existent member", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/test-org/members/non-existent-member",
				{
					method: "DELETE",
				},
			);
	...
	describe("PATCH /internal/organizations/:id/members/:memberId", () => {
		it("validates role is required", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/test-org/members/test-member",
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({}),
			...
			const body = (await response.json()) as ApiResponse<unknown>;
			expect(body.success).toBe(false);
			expect(body.error).toBe("Invalid role. Must be owner, admin, or member");
		});
		...
		it("returns 404 for non-existent member", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/test-org/members/non-existent",
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
	...
	describe("DELETE /internal/organizations/:id/invitations/:invitationId", () => {
		it("returns 404 for non-existent invitation", async () => {
			const response = await SELF.fetch(
				"http://local.test/internal/organizations/test-org/invitations/non-existent",
				{
					method: "DELETE",
				},
			);
